    ...
    <tbody id="inventoryTableBody">
        ...
        <tr>
            ...
            <td>
                        ...
                        <button class="btn btn-custom-edit btn-sm" data-toggle="modal" data-target="#ModalEdit{{ $item->iInvPK}}">
                            <i class="fa fa-edit"></i>
                        </button>
                        
                        <form action="{{ route('inventory.destroy', $item->iInvPK) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
...
<!-- Edit Modal -->
  <div class="modal fade" id="ModalEdit{{ $item->iInvPK}}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $item->iInvPK}}" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <form action="{{ route('inventory.update', $item-> iInvPK) }}" method="POST">
        ...
        <div class="modal-body">
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" class="form-control" id="cInvCode{{ $item->iInvPK}}" name="cInvCode" value="{{ $item->cInvCode }}" maxlength="6" required>
            </div>
            
            <!-- Inventory Name -->
            <div class="form-group mb-3">
                <label for="cInvName{{ $item->iInvPK}}" class="form-label">INVENTORY NAME</label>
                <input type="text" class="form-control" id="cInvName{{ $item->iInvPK}}" name="cInvName" value="{{ $item->cInvName }}" maxlength="100" required>
            </div>
            
            <!-- Inventory Type -->
            <div class="form-group mb-3">
                <label for="cInvType{{ $item->iInvPK}}" class="form-label">INVENTORY TYPE</label>
                <input type="text" class="form-control" id="cInvType{{ $item->iInvPK}}" name="cInvType" value="{{ $item->cInvType }}" maxlength="50" required>
            </div>
            
            <!-- Unit of Measure -->
            <div class="form-group mb-3">
                <label for="iInvUom{{ $item->iInvPK}}" class="form-label">UNIT OF MEASURE</label>
                <input type="text" class="form-control" id="iInvUom{{ $item->iInvPK}}" name="iInvUom" value="{{ $item->iInvUom }}" maxlength="10" required>
            </div>
