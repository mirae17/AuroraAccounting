        ...
        <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
                @foreach($suppliers as $supply)
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn btn-custom-edit btn-sm" data-toggle="modal"
                                data-target="#ModalEdit{{ $supply->iSuppPk }}">
                                <i class="fa fa-edit"></i>
                            </button>

                            <!-- Delete Form -->
                            <form action="{{ route('suppliers.destroy', $supply->iSuppPk) }}" method="POST"
                    ...
                    <!-- Modal Edit -->
                    <div class="modal fade" id="ModalEdit{{ $supply->iSuppPk }}" tabindex="-1" role="dialog"
                        aria-labelledby="ModalEditLabel{{ $supply->iSuppPk }}" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="ModalEditLabel{{ $supply->iSuppPk }}">Edit Payment Method
                                    </h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                ...
                                <form action="{{ route('suppliers.update', $supply->iSuppPk) }}" method="POST">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group mb-3">
                                            <label for="iSuppCode" class="form-label">SUPPLIER CODE</label>
                                            <input type="text" class="form-control" name="iSuppCode" maxlength="6"
                                                value="{{ $supply->iSuppCode }}" required>
                                        </div>
                                        <!-- Description -->
                                        <div class="form-group mb-3">
                                            <label for="iSuppDesc" class="form-label">SUPPLIER DETAILS</label>
                                            <input type="text" class="form-control" name="iSuppDesc" maxlength="50"
                                                value="{{ $supply->iSuppDesc }}" required>
                                        </div>
                                        <div class="form-group mb-3">
                                            @if(Auth::user()->role === 'system admin')
                                                <label for="company_id{{ $supply->iSuppPk }}" class="form-label">COMPANY</label>

                                                <select class="form-control" id="company_id{{ $supply->iSuppPk }}"
